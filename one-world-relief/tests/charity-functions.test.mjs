...
test("home page renders a continuous completed-case photo flow from project data", async () => {
  ...
  assert.match(siteJs, /Array\.from\(\{ length: 6 \}, \(\) => projects\)\.flat\(\)/);
  ...
  assert.match(siteCss, /@keyframes case-card-float/);
  assert.match(siteCss, /@keyframes case-photo-drift/);
  assert.match(siteCss, /@keyframes case-light-sweep/);
