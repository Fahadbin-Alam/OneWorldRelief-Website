...
### 2026-08-28 Mobile Navigation and Donation Fit-and-Finish (Released)
- Mobile navigation across the public site:
  - Projects, Share, About, Contact, Zakat, and Cases 001-009 now use the same accessible phone drawer already present on Home and Donate. Desktop navigation remains unchanged. The phone header is one compact `62-68px` row instead of the previous two-row layout that measured about `118px` and consumed too much landscape height.
  - Every drawer retains all four destinations, the correct current-page state, 44px link targets, Escape/outside-click closing, and link-click closing. At `420px` and below, the drawer spans the usable phone width. Brand text is hidden at `380px` and below while the recognizable logo remains, eliminating the measured Menu/brand collisions at 360-375px.
  - Zakat keeps its language selector and donation action in the same compact row. Its English action is shortened to `Donate`; the 320px header cleanly fits logo, Menu, full `English`, and Donate. Arabic was rechecked at `320x700` with `dir="rtl"`, translated controls, and zero horizontal overflow.
  - The Projects category chips now form a touch-scrollable single row with the visual scrollbar hidden. The Contact email card drops its decorative arrow and uses a compact address treatment on narrow phones so the full public email stays on one line at 320px.
- Earlier, simpler donation actions:
  - The phone homepage order is now introduction -> unrestricted donation -> Case 004 evidence photo -> source details. At `390x844`, the donation card begins at `y=478` and its `Donate now` button ends at `y=776`, so the primary action is visible in the first screen while the verified counts remain above it.
  - The phone action hub is now a restrained white-card list rather than three oversized gradient panels. Its live 390px height fell from about `1,509px` to `917px`; individual cards measure roughly `199-216px` while retaining their descriptions and 44px actions.
  - The homepage footer keeps the verified brand, EIN, tax wording, contact details, Explore links, Ways to Give links, and donation action, but the two link groups remain side-by-side on phones. Its 390px height fell from about `1,384px` to `1,053px`. The complete homepage fell from about `5,521px` to `4,528px` without removing project proof or official details. The phone faith-reminder section was reduced to `450px` and continues to use its poster rather than loading the background video.
  - On Donate, the redundant visible `Amount` heading is now screen-reader-only and the small repeated page eyebrow is removed on phones. Name and email share a row from 350-599px, with a plain `Email address` placeholder; below 350px they stack. At `360x800`, the complete `Donate Now` button ends at `y=795`, inside the first screen. At 320px the stacked version remains readable, uses 16px inputs, and has no horizontal overflow.
- Cache, Stripe, Sheets, and data boundaries:
  - All public HTML now requests `one-world-relief-responsive-v1.css`, internally mapped by `_redirects` to the reviewed shared stylesheet. This unique path prevents an older service worker from serving stale layout CSS beside the new mobile markup. Offline app-shell cache advanced from `owr-offline-v31` to `owr-offline-v32` and precaches the new path.
  - Checkout and webhook Functions were not changed. Server-owned donation rules, Stripe product/metadata derivation, webhook signatures, receipt behavior, Google Sheets `Donations (2026)` A:H mapping, duplicate protection, formula neutralization, and the Zakat privacy boundary remain unchanged.
  - Only empty Checkout validation probes were sent to preview and canonical production; both returned the expected `400`. No real Checkout Session, payment, signed webhook, receipt email, or Google Sheets row was created.
- Verification and release:
  - Feature commit `8b9d97b7491c62ba1fce5c4aa3ca790c296d0749` (`Polish mobile navigation and donation flow`) was pushed to `origin/charity-frontend-redesign` before deployment.
  - The complete regression suite passed `43/43`; JavaScript and service-worker syntax checks passed; CSS parsed through esbuild; and `git diff --check` passed. The safe Cloudflare workflow staged `71` public assets plus `5` Pages Function sources and compiled them with Wrangler `4.127.1`.
  - A browser audit covered all 18 public routes at both `390x700` and `320x700` (36 route/viewport combinations) with zero horizontal overflow, zero broken loaded images, no visible form control below 16px, no undersized tested button/summary/drawer target, and the compact drawer active on every content route. Representative phone-landscape checks at `568x320` also passed across Home, Donate, Projects, Share, About, Contact, Zakat, and Case 004.
  - Preview deployment `64dfc30f-6280-4c22-a401-3d93d3adc747` succeeded at `https://64dfc30f.trying-8o0.pages.dev`, branch `charity-frontend-redesign`, source `8b9d97b`.
...
### 2026-08-29 Qur'an Giving Quote and Blue Donation Card (Released)
- Replaced the generic `Give what you can. See the work.` homepage headline with the exact Clear Quran excerpt `And whatever you spend in charity, He will compensate you for it.` A visible `Qur'an 34:39 · The Clear Quran` citation links to `https://quran.com/34/39`; the former headline no longer appears in the active homepage.
- Restored the established One World Relief blue treatment without returning to glossy or overly decorative styling. The hero uses a soft blue field, while the donation card uses a restrained pale-blue gradient, blue border/accent, clearer `Choose your amount` heading, and solid deep-blue `Donate now` action. The real Case 004 proof photo and verified evidence line remain below the first interaction.
- Phone spacing was tightened for the longer quotation. At `390x844`, the primary action ends around `y=780`; at `320x800`, it ends around `y=771`. The citation remains a 44px target. Home, Donate, Projects, Zakat, Contact, About, Share, and Case 009 loaded the new stylesheet at 320px with no horizontal page overflow or undersized tested primary controls.
- All public HTML now requests `one-world-relief-blue-v1.css`, internally mapped to the reviewed shared stylesheet. The unique alias prevents older service-worker CSS from being paired with the revised hero markup. Offline app-shell cache advanced from `owr-offline-v33` to `owr-offline-v34`; older aliases remain valid for previously cached pages.
- Stripe Checkout Functions, webhook verification, receipts, donation-program rules, Zakat privacy, project records, and the Google Sheets `Donations (2026)` A:H mapping were not changed. Preview and canonical empty Checkout probes returned the expected safe `400` validation response. No Checkout Session, payment, signed webhook, receipt email, or spreadsheet row was created.
- Feature commit `b1d82de1a03a4207a1ea55789b4c11b1ccc3b461` (`Add Quran giving quote and blue donation card`) and trust-copy follow-up `615d80cce5caca16efba0fe78e8e54f9691259ad` (`Clarify homepage donation wording`) were pushed to `origin/charity-frontend-redesign`. The follow-up replaced the technically broad `Donate any amount through Stripe` sentence with the accurate `Choose an amount and continue securely through Stripe` wording while keeping the minimum out of promotional copy.
- The full regression suite passed `43/43`; JavaScript and service-worker syntax checks passed; CSS parsed through esbuild; `git diff --check` passed; and the browser console remained free of warnings and errors.
- Final preview deployment `458a4dd8-70c7-43a9-bf74-bb09d478cdde` succeeded at `https://458a4dd8.trying-8o0.pages.dev`, branch `charity-frontend-redesign`, source `615d80c`.
- Final production deployment `bc218c6f-c3de-4175-a91a-d4bc7f35f0bb` succeeded at `https://bc218c6f.trying-8o0.pages.dev`, branch `main`, source `615d80c`. Canonical desktop and phone browser checks confirmed the exact quote/source, corrected Stripe wording, blue cache-safe stylesheet, visible first-screen donation action, and active safe Checkout validation.

### 2026-08-29 Coming-Soon Elder-Care Case 010 (Released)
- Added Case 010 as `A Safer Elder-Care Home`, categorized as `Elder Care` and clearly marked `Coming Soon`. The planned work is described as a renovation intended to improve the safety, comfort, and dignity of a residential home serving vulnerable older adults without reliable family support.
- The case uses the existing generated `Coming Soon 010` banner. No location, resident count, building condition, repair list, budget, schedule, completion statement, photo, or video was invented. The page says those details and approved media will be published only after assessment and confirmation. Future resident media must respect consent, health privacy, room/location privacy, and other safeguarding boundaries.
- The Projects board now reports `10 projects`, `7 completed`, `0 active`, and `3 coming soon`, and includes an `Elder Care` category chip. The homepage proof reel remains completed-only, so Case 010 correctly does not appear among completed work.
- Case 010 does not claim to have a fixed-price or restricted donation program. Its action is labeled `Give where needed most` and opens the existing unrestricted donation flow with `referrer=case-010`. Stripe metadata and the Google Sheets A:H notes mapping preserve `Referrer Case: case-010`, while the purpose remains the honest `General Fund`. No Checkout Session, payment, signed webhook, receipt email, or spreadsheet row was created during verification.
- Updated the project-media README with the no-public-media state, moved the intake template's hypothetical next-case examples from 010 to 011, and expanded page-count, mobile, project-data, Stripe-metadata, and Sheets-note regression coverage for Case 010.
- Feature commit `41a71dee000d680195cf498ee1db753d45d8115b` (`Add coming-soon elder care Case 010`) was pushed to `origin/charity-frontend-redesign`. The full regression suite passed `43/43`; project and site JavaScript syntax checks passed; `git diff --check` passed; and the safe deployment artifact contained `72` public assets plus `5` Pages Function sources.
- Browser QA passed on local, preview, and canonical production at desktop `1440x900` and phone `390x844`. Projects and Case 010 had zero horizontal overflow; both phone actions measured at least 44px; the canonical donation handoff retained `program=unrestricted` and `referrer=case-010`; and the browser console had no warnings or errors.
- Preview deployment `64752dbb-41be-414a-9a36-c6e5cb98a962` succeeded at `https://64752dbb.trying-8o0.pages.dev`, branch `charity-frontend-redesign`, source `41a71de`.
- Production deployment `b20f9afe-735c-4fda-ae14-8b904de61d15` succeeded at `https://b20f9afe.trying-8o0.pages.dev`, branch `main`, source `41a71de`. Canonical checks on `https://one-world-relief.org/projects` and `/projects/case-010` confirmed the final title, coming-soon status, general-giving semantics, responsive fit, and safe empty Checkout validation response `400`.

### 2026-08-30 Private-by-Default Supporter Board and Unified Blue Homepage (Released)
- Replaced the former three-card homepage action hub with a two-column public supporter area: `Top supporters` (up to ten people) and `Recent public support` (up to ten gifts). The first three ranks retain small gold, silver, and bronze badges; the page otherwise stays in the established One World Relief blue system. On phones the two panels stack into one column.
- Public recognition is explicit and off by default. The Donate form has a separate `Show me on the public supporter board` checkbox and a conditional 2-40 character public display-name field. Choosing anonymity turns public recognition off, and choosing public recognition clears anonymity. The public name, amount, cause, date, and public total are disclosed beside the checkbox before consent.
- The Checkout Function accepts only literal `supporter_board_opt_in: true`, validates the display name, rejects public/anonymous conflicts, and copies the consent into Stripe session and payment metadata. Missing values, string booleans, malformed names, and historical donations remain private.
- The Stripe webhook records only paid one-time sessions, supports `checkout.session.async_payment_succeeded`, and adds trusted supporter markers to column H. The connected Google Sheet remains exactly A:H: `Donation ID | Date | Donor Name | Amount ($) | Purpose/Fund | Method | Receipt ID | Notes`. No schema or formula range changed.
- New public endpoint `GET /charity/donors/leaderboard` reads a bounded A1:H5000 range with read-only Google Sheets scope. It combines opted-in gifts by normalized email in memory, but returns only public display name, total/count, recent amount, cause, and completion time. It never returns email, notes, hashes, Stripe IDs, receipt IDs, or Sheet credentials. Historical rows without `Supporter Board: Yes` do not appear.
- The live board currently returns zero entries because no existing donor was retroactively opted in. This is intentional; future paid donors appear automatically only when they choose public recognition. Refunds and chargebacks are not yet subtracted automatically, email ownership is not authenticated, and consent removal requires changing the Sheet marker to `Supporter Board: No` (with up to roughly two minutes of cache delay).
- Removed the pale/white visual break shown in owner feedback. The homepage hero, completed-project reel, and supporter area now share one continuous `#d9f3fa` blue canvas. Home and Donate use the cache-safe `one-world-relief-community-v2.css` alias, and the offline cache advanced to `owr-offline-v36`.
- Feature commit `516f2312cb44142661c06ca949879c8d3017505c` (`Add private-by-default supporter board`) was pushed to `origin/charity-frontend-redesign`. The full regression suite passed `47/47`; JavaScript/service-worker syntax checks, mirrored Function hashes, `git diff --check`, and the safe deployment artifact check passed. The artifact contained `73` public assets and `6` Pages Function source files.
- Production deployment `d651978b` succeeded at `https://d651978b.trying-8o0.pages.dev`, branch `main`, source `516f231`. Canonical checks on `https://one-world-relief.org` confirmed the new stylesheet, supporter markup, continuous blue background, public endpoint HTTP `200`, strict public response without private keys, and zero horizontal overflow at desktop and `390px`. Live Donate QA confirmed unchecked-by-default consent, conditional public-name disclosure, and anonymous/public mutual exclusion. No Checkout Session, payment, receipt email, or Google Sheets row was created during verification.

### 2026-09-03 Calmer Homepage Hero and Mobile Giving Flow (Released)
- Reduced the opening Qur'an quotation from a maximum `3.8rem` to `3.1rem`, increased its line-height, widened its readable measure, and shortened the supporting proof copy to `7 completed cases` and `2 orphan students supported`. The exact Qur'an 34:39 quotation, source link, Case 004 evidence, verified-count boundary, and One World Relief blue canvas remain unchanged.
- Tightened the donation card without shrinking its usable controls: its heading, padding, internal gaps, amount field, trust row, and overall width are calmer, while the amount field and primary action remain comfortably above the 44px touch baseline. The supporting sentence is hidden on phones because the amount field and button already explain the first step.
- Tablet and phone order is now consistently quotation -> donation -> documented Case 004 photo -> verification links. The stack is limited to `560px` on tablets, the brand wordmark hides below `420px` to protect the phone header, and all hero grid children explicitly allow safe shrinking.
- Rendered QA passed locally at `1440x900`, `768x900`, `390x844`, and `320x800`, then passed again on canonical production at `390x844`. The quote fell from about `61px` to `50px` on desktop and from about `41px` to `32px` at 390px. The phone hero became roughly `180px` shorter, all tested blocks stayed inside the viewport, the intended ordering held, and browser logs contained no warnings or errors.
- Home and Donate now request the cache-safe `one-world-relief-community-v3.css` alias, and the offline cache advanced from `owr-offline-v36` to `owr-offline-v37`. Older stylesheet aliases remain available for already-cached pages.
- This release changes presentation only. Stripe Checkout, server-owned amount rules, webhook processing, receipts, the public supporter endpoint, Google Sheets A:H recording, Zakat privacy, and project data were not changed. No Checkout Session, payment, signed webhook, receipt email, or spreadsheet row was created.
- Feature commit `a1e9c79` (`Refine homepage hero spacing and mobile flow`) was pushed to `origin/charity-frontend-redesign`. The complete regression suite passed `47/47`; JavaScript and service-worker syntax checks plus `git diff --check` passed. The safe Cloudflare workflow staged `73` public assets and `6` Pages Function sources and compiled the Functions bundle with Wrangler `4.129.0`.
- Production deployment `9dcd2497` succeeded at `https://9dcd2497.trying-8o0.pages.dev`, branch `main`, source `a1e9c79`. Both the immutable deployment and `https://one-world-relief.org` returned HTTP `200` for the homepage and the v3 stylesheet; canonical browser QA confirmed the new quote sizing, compact card, mobile ordering, and zero horizontal overflow.
