...
### 2026-08-17 Multilingual Zakat Calculator (Release Pending)
...
- Release state at the time of this entry:
  - Frontend, mirrored Checkout Functions, mirrored Stripe webhooks, and regression coverage are being completed and reviewed. The offline service-worker cache is planned to advance from `owr-offline-v8` to `owr-offline-v9`, with `zakat.html` and `zakat-calculator.js` added to the app shell.
  - Full regression, syntax, CSS, link, responsive/RTL browser, preview, production, and canonical-domain verification are pending. No Cloudflare deployment ID or release commit is recorded yet; append those facts only after the checks actually succeed.
