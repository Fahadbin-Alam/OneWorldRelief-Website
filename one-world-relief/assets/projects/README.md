...
- `case-004/` contains the `thumbnail`, `main`, and `proof` JPEGs plus the browser-safe `primary` MP4 used by `projects/case-004.html`. The public files document a Korbani meal service on June 24, 2026, and have device and precise-location metadata removed. The original 255 MB phone video remains outside the website, and the retained legacy placeholder is not referenced publicly.
